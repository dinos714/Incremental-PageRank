...
这在数学上是一个马尔可夫过程，且这样的随机冲浪过程一直进行下去，某一个网页被访问到的概率就是它的 PageRank。那么如果当前网页是 $j$，下一次访问到网页 $i$ 的概率的计算方式就是：
...
幂法是求解大规模稀疏矩阵的主特征向量的可靠、唯一的选择。本节论证 PageRank 向量为何恰好是转移矩阵 $A$ 的主特征向量，从而为幂法的应用提供理论依据。

#### PageRank 向量作为主特征向量

由式 $(5)$ 可知 $A\vec{x} = \vec{x}$，即 PageRank 向量 $\vec{x}$ 是矩阵 $A$ 对应于特征值 $\lambda_1 = 1$ 的特征向量。为证明 $1$ 是主特征值（模最大的特征值），我们证明 $A$ 是列随机矩阵。

对于任意列 $j$，列和为：

$$
\sum_{i=1}^{n} a_{ij} = \sum_{i=1}^{n} \left(\frac{p g_{ij}}{c_j} + \frac{1-p}{n}\right)
$$

当 $c_j \neq 0$ 时，$\sum_i g_{ij} = c_j$，因此：

$$
\sum_{i=1}^{n} a_{ij} = p\frac{c_j}{c_j} + n\cdot\frac{1-p}{n} = p + (1-p) = 1
$$

当 $c_j = 0$（悬挂节点）时，$g_{ij} = 0 \ (\forall i)$，由 $f_j = 1/n$ 及 $d_{jj}=1$，列和同样为 $1$（验证从略）。故 $A$ 每一列的和都是 $1$，$A$ 是列随机矩阵。

根据 Perron–Frobenius 定理：对于非负不可约矩阵，谱半径 $\rho(A)$ 等于其最大正特征值，且存在对应的正特征向量。对于列随机矩阵 $A$，$\rho(A) \le \|A\|_1 = 1$（列和范数），而 $\vec{e}^T A = 1\cdot\vec{e}^T$ 说明左特征值 $1$ 确实存在，因此 $\rho(A) = 1$，特征值 $1$ 就是主特征值。

当阻尼系数 $p < 1$ 时，随机跳转项 $(1-p)\vec{e}\vec{f}^T / n$ 使得 $A$ 变为正矩阵（所有元素大于 0），保证了不可约性和本原性。此时除 $\lambda_1 = 1$ 外，所有特征值满足 $|\lambda_i| \le p < 1 \ (i \ge 2)$。因此主特征值与次主特征值之间存在严格分离，称为谱间隙（Spectral Gap），其大小为 $1-p$。

综上，PageRank 向量 $\vec{x}$ 是 $A$ 的非负归一化主特征向量（对应于 $\lambda_1=1$），且是唯一满足 $\sum_i x_i = 1$ 的特征向量。

#### 幂法的收敛性

幂法迭代 $\vec{x}^{(k+1)} = A\vec{x}^{(k)}$。设初始向量 $\vec{x}^{(0)}$ 在特征基 $\vec{v}_1, \vec{v}_2, \dots, \vec{v}_n$ 下展开：

$$
\vec{x}^{(0)} = c_1\vec{v}_1 + \sum_{i=2}^{n} c_i\vec{v}_i
$$

经 $k$ 次迭代后：

$$
\vec{x}^{(k)} = A^k\vec{x}^{(0)} = c_1\lambda_1^k\vec{v}_1 + \sum_{i=2}^{n} c_i\lambda_i^k\vec{v}_i = c_1\cdot 1^k\cdot\vec{v}_1 + \sum_{i=2}^{n} c_i\lambda_i^k\vec{v}_i
$$

由于 $|\lambda_i| \le p < 1 \ (i \ge 2)$，非主特征值对应分量以至少 $p^k$ 的速率衰减。因此：

$$
\vec{x}^{(k)} \to c_1\vec{v}_1 = \vec{x} \quad (k \to \infty)
$$

收敛到主特征向量的速率由 $p$ 主导：$p$ 越接近 $1$，谱间隙 $1-p$ 越小，收敛越慢。这一特性在后续实验中（阻尼系数敏感性分析）得到了直接验证。

幂法成为 PageRank 计算的事实标准取决于以下三点核心优势：

1. **仅需矩阵-向量乘法**：无需显式存储或分解大规模稀疏矩阵，每次迭代只需计算 $A\vec{x} = pG(D\vec{x}) + \vec{e}(\vec{f}^T\vec{x})$，时间复杂度 $O(|V| + |E|)$，空间复杂度仅需存储稀疏分量 $G, D, \vec{f}$。

2. **对稀疏特性的保留**：$G$ 以 CSR 格式存储，仅保留非零边信息。稠密矩阵 $A$ 永远不需要被显式构造——实际上 $A$ 是稠密的（随机跳转项保证了每对节点间都有非零概率），强行存储将导致 $O(N^2)$ 的内存灾难。

3. **数值稳定性与可靠性**：相比于 Arnoldi / Lanczos 等 Krylov 子空间方法对矩阵性质（对称性、正定性）的偏好，幂法对列随机矩阵的非对称、非负结构无任何额外要求，且在谱间隙实际存在的情况下收敛是理论保证的。
