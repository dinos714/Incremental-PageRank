...
实验数据表明，增量式算法（特别是 Local Push）在阻尼系数适中（约0.85-0.90）且精度要求较高（$10^{-12}$）的动态网络更新场景中具有最强的竞争力。这契合实际工业界对 PageRank 实时性与准确性的双重需求。与此同时，在 $p$ 极高或扰动过大的极端情况下，系统本身的数值不稳定性使增量算法的边际收益受到压缩。