...
def run_experiments_and_plot(data_file_path, p, add_ratios, drop_ratios, variable, result_save_path):
    ...
    plt.plot(x_values, delta_x_norm1_list, marker='o', linestyle='-', label=r'$||\Delta x||_1$')
    plt.plot(x_values, theoretical_bound_list, marker='x', linestyle='--', label=r'Theoretical Bound $\frac{p}{1-p}||\Delta M||_1$')    
    plt.xscale('log')
    plt.yscale('log')    
    plt.xlabel(f'Perturbation Ratio ({variable.replace("_", " ").title()})')
    plt.ylabel('L1 Norm (Log Scale)')
